'''
This script analyses a pdb file given in entry and extract alpha
carbon atoms on the precised chain. Then, it cuts the protein 
in protein units (PUs) and calculates three criteria :
    - partition index (PI)
...
At the end, two .txt files are created :
    ...
    - chain_name.txt : which contains best significant PUs
Also, .png files are created that show boundaries of PUs 
on the contacts matrix.
'''
import re
import sys
...
class Atome :
    ...
    def __init__(self,line):
        '''
        Initializes an Atomes instance
        '''
...
class PU :
    ...
    Attributes :
        ...
        signif (str) : significant criteria
    '''
    def __init__(self, begin, end, size) :
        '''
        Initializes a PU instance
        '''
        self.begin = begin
        self.end = end
        self.size = size
...
def calculate_criteria(contacts, begin, min_size, max_size, list_ss) :
    ...
    '''
    list_PU = []
    for end in range((begin+min_size-1), (begin+max_size)) :
        size = end - begin + 1
...
def find_PU(PUs) :
    ...
    Finds PUs based on PI and sigma :
        - the p-value of PI should be lower than 0.05 and 
          the z-score should be higher than 0
        - the p-value of sigma should be lower than 0.05 
          and the z-score should be lower than 0
...
def create_file(list_PU, name, namefile, min_size, option, chain) :
    ...
    - k
    '''
    file = open("./resultPU/"+name+"/"+namefile, option)
...
def main() :
    ...
    for pu in list_best_PU :
        ...
        plt.savefig("resultPU/"+name+"/"+chain+"_"+name+"_"+str(pu.begin)+".png")
        plt.show()
